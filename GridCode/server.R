...
shinyServer(function(input, output) {
  ...
  output$MainBody<-renderUI({
    fluidPage(
      ...
      tags$script("$(document).on('click', '#Main_table cell', function () {
                            Shiny.onInputChange('cellClick',this.id);
    ...
    DT[["Actions"]]<-
      paste0('
             <div class="btn-group" role="group" aria-label="Basic example">
                ...
                <button type="button" class="btn btn-secondary "id=alert_',1:nrow(vals$Data),'>View</button>
             </div>
  ...
  modal_alert<-modalDialog(
    fluidPage(
      h3(strong("Row message"),align="center"),
      hr(),
      textOutput("row_alert")
    ), size="l")
  
  modal_text<-modalDialog(
    fluidPage(
      h3(strong("Row message"),align="center"),
      hr(),
      textOutput("cell_value")
    ), size="l")
  ...
  observeEvent(input$lastClick,
               {
                 ...
                 else if (input$lastClickId%like%"alert")
                 {
  ...
  observeEvent(input$cellClick,{
    if (input$Main_table_cell_clicked){
  ...
  output$row_alert<-renderText({
    old_row=vals$Data[as.numeric(gsub("alert_","",input$lastClickId))]
    old_row$Forecasted_Growth
    })
  
  
  output$cell_value<-renderText({
    input$Main_table_cell_clicked$value
  })
