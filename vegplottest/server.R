...
shinyServer(function(input, output,session) {
  ...
  output$map <- renderLeaflet({
    ...
    if ((!is.null(match_data$matches))&&(check_infile()$env_present)) {
      ...
      unmatchedplots<-unmatchedplots %>% filter((unmatchedplots$lat>=min(dtfinal$Latitude)-0.6),
                                                (unmatchedplots$lat<=max(dtfinal$Latitude)+0.6),
                                                (unmatchedplots$long>=min(dtfinal$Longitude)-0.5),
                                                (unmatchedplots$long<=max(dtfinal$Longitude)+0.5))
      ...
      m <- leaflet(data=dtfinal )%>% addTiles(group = "Terrain") %>%
       ...
       addAwesomeMarkers(icon=oehblueicon, lat = dtfinal$Latitude, lng = dtfinal$Longitude,
                         layerId = dtfinal$sites, label = dtfinal$sites, 
                         labelOptions = labelOptions(noHide = T, direction = "bottom"),
                         popup = ~paste("<b>Site No:</b>", dtfinal$sites,"<br/><b>Lat:</b>", 
                                        dtfinal$Latitude," <b>Long:</b>", dtfinal$Longitude, 
                                        "<br/><b>Elevation (m):</b>", dtfinal$Elevation, 
                                        "<br/><b>Annual Rainfall (mm):</b>",dtfinal$RainfallAnn,
                                        "<br/><b>Annual Mean Temperature (°C):</b>",dtfinal$TempAnn,
                                        "<br/>", pctstats)
                         ) %>%
       addCircles(radius= 100, lat = ~matchedplots$lat, lng = ~matchedplots$long,
                  ...
                  data = matchedplots,
                  popup = ~paste("<b>PCT ID:</b>",matchedplots$pctid,
                                 "<br/><b>PCT Name:</b>", matchedplots$pctname,
                                 "<br/><b>PCT Assignment Category:</b>",matchedplots$pctassignmentcategory,
                                 "<br/><b>Site No:</b>",matchedplots$siteno,"<br/><b>Survey Name:</b>",
                                 matchedplots$surveyname  ,"<br/><b>Lat:</b>",matchedplots$lat,
                                 " <b>Long:</b>",matchedplots$long,"<br/><b>Elevation (m):</b>",
                                 matchedplots$elevation,"<br/><b>Annual Rainfall (mm):</b>",
                                 matchedplots$rainfall,"<br/><b>Annual Mean Temperature (°C):</b>",
                                 matchedplots$temp)
                  ) %>%
        addCircles(radius= 50, lat = ~unmatchedplots$lat, lng = ~unmatchedplots$long,
                   layerId = ~unmatchedplots$siteno,
                   color = ~UnMatchedCol(unmatchedplots$pctid),
                   fillOpacity = 0.5,
                   group = groupName,
                   data = unmatchedplots,
                   popup = ~paste("<b>PCT ID:</b>", unmatchedplots$pctid,
                                  "<br/><b>PCT Name:</b>", unmatchedplots$pctname,
                                  "<br/><b>PCT Assignment Category:</b>",
                                  unmatchedplots$pctassignmentcategory,
                                  "<br/><b>Site No:</b>",unmatchedplots$siteno,
                                  "<br/><b>Survey Name:</b>", unmatchedplots$surveyname  ,
                                  "<br/><b>Lat:</b>",unmatchedplots$lat," <b>Long:</b>",
                                  unmatchedplots$long,"<br/><b>Elevation (m):</b>",
                                  unmatchedplots$elevation,"<br/><b>Annual Rainfall (mm):</b>",
                                  unmatchedplots$rainfall,"<br/><b>Annual Mean Temperature (°C):</b>",
                                  unmatchedplots$temp)
                   ) %>%
       hideGroup(groupName) %>%
       addLayersControl(
     ...
     if (nrow(dtfinal)>1){
       ...
       m <- m %>% fitBounds(~min(dtfinal$Longitude), ~min(dtfinal$Latitude), 
                            ~max(dtfinal$Longitude), ~max(dtfinal$Latitude))
     }
